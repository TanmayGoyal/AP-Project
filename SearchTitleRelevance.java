...
public class SearchTitleRelevance extends DefaultHandler {
	...
	/**
   	...
   	*/
	public SearchTitleRelevance (String title, String year, ArrayList<AuthorNames> authorEntities) {
		recTitle = title;
	...
	/**
   	...
   	*/
	public SearchTitleRelevance (String title, String startYear, String endYear, ArrayList<AuthorNames> authorEntities) {
	...
	* been explicity added by programmer to check from the XML file.*/
	...
	public String[][] getArray() {
		String[][] arr = new String[q1Display.size()][9];

		for (int i = 0;i<q1Display.size();i++) { //SNO, AUTHORS, TITLE, PAGES, YEAR, VOLUME, JOURNAL, URL, RATIO
			...
			Double x = (double)recTitle.length()/(double)q1Display.get(i).title.length();
			arr[i][8] = Double.toString(x);
		}

		Arrays.sort(arr, new Comparator<String[]>() {
            @Override
            public int compare(final String[] entry1, final String[] entry2) {
                double time1 = Double.parseDouble(entry1[8]);
                double time2 = Double.parseDouble(entry2[8]);
	...
	* on the table. It also sorts the data in the 2D array based on relevance.*/
}